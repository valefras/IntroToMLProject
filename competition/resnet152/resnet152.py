...
def configure_subparsers(subparsers):
  ...
  """
    parser = subparsers.add_parser(
        "resnet152", help="Train and Test the resnet152 model")
    parser.add_argument(
...
def main(args):
    ...
    model = resnet152(torch.hub.load('pytorch/vision:v0.10.0', 'resnet152', pretrained=pretrained),
            None,None,model_transform,test_transform,"resnet152","scraped_fixed",100,True,pretrained)

    if(args.test != None):
        if not pretrained:
            if args.test == "latest":
                path_model = utils.get_latest_model("resnet152")
            else:
                path_model = utils.get_path(
                    f"../../models/resnet152/resnet152-{args.test}.pth")
    else:
